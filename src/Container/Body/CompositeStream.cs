...
namespace Pawod.MigrationContainer.Container.Body
{
    ...
    public class CompositeStream : Stream
    {
        ...
        private long CurrentRemaining => CurrentSubStream.Length - CurrentSubStream.Position;

        private Stream CurrentSubStream => _subStreams[_currentIndex];

        private long TotalRemaining => _length - Position;

        public CompositeStream(List<SubStream> subStreams)
        {
            if (subStreams == null) throw new ArgumentNullException(nameof(subStreams));
        ...
        {
            foreach (var subStream in _subStreams) { subStream.Flush(); }
        }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            foreach (var subStream in _subStreams) { subStream.Dispose(); }
            _subStreams = null;
        }
