namespace DataMigrator.FileSystem
{
    ...
    public class JunctionPoint : Win32Api
    {
        ...
        {
            Link = linkedDir;
            Target = new DirectoryInfo(Path.GetFullPath(GetTarget(linkedDir.FullName)));
        }
        ...
        {
            Link = new DirectoryInfo(linkedDir);
        ...
        public SafeFileHandle CreateGetFileHandle(bool overwrite = true)
        {
            ...
            try
            {
                handle = OpenReparsePoint(Link.FullName, EFileAccess.FILE_WRITE_ATTRIBUTES);
                var targetDirBytes = Encoding.Unicode.GetBytes(NON_INTERPRETED_PATH_PREFIX + Target.FullName);
        ...
        public static bool Exists(DirectoryInfo directoryInfo)
        {
            ...
            using (var handle = OpenReparsePoint(directoryInfo.FullName, EFileAccess.GENERIC_READ))
            {
        ...
        public static IList<JunctionPoint> FindJunctions(DirectoryInfo directory,
